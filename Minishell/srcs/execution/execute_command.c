...
/*
AQUI ES LO QUE HAY QUE MIRAR
hay que hacer que espere a todos los procesos no solo al ultimo
*/
void	execute_commands(t_data *d)
{
	...
	while (d->cmds)
	{
		...
		//aqui he hecho los cambios
		if (first_pid == 0)
			first_pid = d->last_pid;
	}
	
	waitpid(first_pid, &d->status, WUNTRACED);
